...
Game::Game(){
    ...
    SmallGun = new Gun(Global.GUN_DAMAGE,Global.FAST_BULLET_SPEED,Global.FAST_BULLET_SIZE, Global.NUMBER_OF_BULLETS);
...
void Game::run(){
    ...
    while (window->pollEvent(clsevent))
    {
        ...
        {
            window->close();
        } else  if(clsevent.type == sf::Event::TextEntered && gameScreen == "gameover")
        {   if (clsevent.text.unicode < 128){
                //std::cout << "ASCII character typed: " << static_cast<char>(clsevent.text.unicode) << std::endl;
                playerInput +=clsevent.text.unicode;
    ...
    if (gameScreen == "menu"){
        ...
        this->runLeaderboardInput();
    }else if (gameScreen == "tryagain"){
        this->runGameOver();
    }
...
void Game::runLeaderboardInput(){
    //playerText.setString("Exit Game");
    text.setString("You Scored: " + std::to_string(score));
    text.setOrigin(sf::Vector2f(text.getLocalBounds().width/2.0f,0));
    text.setPosition(sf::Vector2f(Global.GW_X/2,Global.GW_Y/2 -200));

    beginGameButtonText.setString("Please type your name");
    beginGameButtonText.setOrigin(sf::Vector2f(beginGameButtonText.getLocalBounds().width/2.0f,0));
    beginGameButtonText.setPosition(sf::Vector2f(Global.GW_X/2,Global.GW_Y/2 -100));

    playerText.setOrigin(sf::Vector2f(playerText.getLocalBounds().width/2.0f,0));
    playerText.setPosition(sf::Vector2f(Global.GW_X/2,Global.GW_Y/2 - 50));

    exitGameButtonText.setString("Next");
    exitGameButtonText.setOrigin(sf::Vector2f(exitGameButtonText.getLocalBounds().width/2.0f,0));
    exitGameButtonText.setPosition(sf::Vector2f(Global.GW_X/2,Global.GW_Y/2 + 200));
...
void Game::runGunMenu(){
    //format text to always be in the centre of the screen and draw
    //Perhaps group this in a function
    text.setString("Please Select a Gun:");
    text.setOrigin(sf::Vector2f(text.getLocalBounds().width/2.0f,0));
    text.setPosition(sf::Vector2f(Global.GW_X/2,Global.GW_Y/2 - 100));
    ...
    // Interaction with the 'Start Game' or 'Exit Game' text
    if(sf::Mouse::isButtonPressed(sf::Mouse::Button::Left) && beginGameButtonText.getGlobalBounds().contains(mousePosition.x, mousePosition.y)){
        fastgun = new Gun(Global.GUN_DAMAGE,Global.FAST_BULLET_SPEED,Global.FAST_BULLET_SIZE, Global.NUMBER_OF_BULLETS);
        SmallGun = fastgun;
        Player1->ChangeGun(fastgun);
        gameScreen = "game";
    }

    if(sf::Mouse::isButtonPressed(sf::Mouse::Button::Left) && exitGameButtonText.getGlobalBounds().contains(mousePosition.x, mousePosition.y)){
        slowgun = new Gun(Global.GUN_DAMAGE,Global.SLOW_BULLET_SPEED,Global.SLOW_BULLET_SIZE, Global.NUMBER_OF_BULLETS);
        SmallGun = slowgun;
        Player1->ChangeGun(slowgun);
        gameScreen = "game";
    }
}
